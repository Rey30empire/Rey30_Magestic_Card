...
const dbPath = path.join(os.tmpdir(), `rey30-int-training-timeout-${Date.now()}-${Math.random().toString(16).slice(2)}.db`);
...
test("training job fails with timeout when runtime budget is exceeded", async () => {
  const env = {
    ...process.env,
    PORT: String(port),
    DB_PATH: dbPath,
  ...
  try {
    ...
    const username = `timeout_user_${Date.now()}`;
    const register = await postJson(
      "/api/auth/register",
      { username, password: "TimeoutPass123!" },
      {
        "x-client-platform": "web"
      }
    );
    ...
    assert.equal(failed.errorMessage, "job timeout");
